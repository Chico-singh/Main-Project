...
if category.empty:
    ...
    st.info("No expense data available to generate AI insights.")
    ...
    st.info(
        f"💸 Total Spending: ₹ {highest['amount']:,.0f}"
    )

    if saving_rate >= 30:

        st.success(
            "🟢 Excellent! Your savings are healthy."
        )

    elif saving_rate >= 15:

        st.warning(
            "🟡 Good! Try increasing your savings."
        )
...
if expense.empty:
    ...
    if not category.empty:
        ...
        if highest["category"] == "Shopping":
            ...
            recommendations.append(
                "🍔 Reduce food expenses by planning meals."
            )

        elif highest["category"] == "Entertainment":
...
if total_income == 0:
    ...
    recommendations.append(
        "💰 Add your income to calculate savings."
    )
...
st.caption(
    "FinSight AI | Dashboard | Developed by Suryansh Singh"
)